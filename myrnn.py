...
maximum_words_in_sentences=0
for i in range(len(labels_in_digits)):
    if len(x_in_words[i])>maximum_words_in_sentences:
        maximum_words_in_sentences=len(x_in_words[i])
for i in range(len(labels_test_in_digits)):
    if len(x_test_in_words[i])>maximum_words_in_sentences:
        maximum_words_in_sentences=len(x_test_in_words[i])
...
for sentence in x_in_words:
    ...
    seqlen_x.append(maximum_words_in_sentences-len(temp))
    for i in range(maximum_words_in_sentences-len(temp)):
        temp.append(0)
    x.append(temp)
for sentence in x_test_in_words:
    temp=[vocabulary[word] for word in sentence]
    seqlen_x_test.append(maximum_words_in_sentences-len(temp))
    for i in range(maximum_words_in_sentences-len(temp)):
        temp.append(0)
    x_test.append(temp)
x=np.array(x)
...
def dynamicRNN(x, seqlen, weights, biases):
    ...
    x = tf.split(0, maximum_words_in_sentences, x)
...
scores = dynamicRNN(embedded_chars, seqlen, weights, biases)
...
with tf.Session() as sess:
    ...
    for i in range(50000):
        ...
        sess.run(train_step,feed_dict={input_x:x[:batch_size],input_y:labels_in_vec[:batch_size],seqlen:seqlen_x[:batch_size]})
        print 'Iterim:',i,'  accuracy:',sess.run(accuracy,feed_dict={input_x:x[:batch_size],input_y:labels_in_vec[:batch_size],seqlen:seqlen_x[:batch_size]}),'  loss:',sess.run(loss,feed_dict={input_x:x[:50],input_y:labels_in_vec[:50],seqlen:seqlen_x[:50]})
        if i%100==0:
            ...
            print sess.run(accuracy,feed_dict={input_x:x_test[:],input_y:labels_test_in_vec[:],seqlen:seqlen_x_test[:]})
